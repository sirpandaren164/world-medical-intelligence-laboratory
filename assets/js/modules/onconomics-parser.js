...
MIW.OnconomicsParser=(function(){
 ...
 function exactSourceNaturalLabel(name,block){
   const source=String(block||"");
   for(const alias of aliasesFor(name)){
     const token=escRe(alias).replace(/\\s+/g,"\\s+");
     const match=source.match(new RegExp("(?:^|[^A-Za-z0-9])("+token+")(?=$|[^A-Za-z0-9])","i"));
 ...
 function sourceFidelityGeneLabel(name,reportedName=""){
   const reported=String(reportedName||"").replace(/\s+/g," ").trim();
   if(name==="Histone-deacetylase-dipeptide"&&/histonedeacetyl/i.test(reported.replace(/\s+/g,"")))return"Histonedeacetylase-";
   return name
 }
 function sourceFidelityFunctionLabel(name,functionCell=""){
   const source=String(functionCell||"").replace(/\s+/g," ").trim();
   const template=MIW.GeneReportSchema?.resolve?.(name)||null;
   const expected=String(template?.function||template?.subGroup||"").replace(/\s+/g," ").trim();
   if(source&&expected&&/\band$/i.test(source)&&expected.toLowerCase().startsWith(source.toLowerCase())&&expected.length>source.length)return expected;
   return source
 }
 ...
 function metadata(pages,fileName){
   const all=pages.map(p=>p.text).join("\n"), summaryRecord=pages.find(p=>/REPORT\s+SUMMARY/i.test(p.text))||pages[0]||null, summary=summaryRecord?.text||all;
   ...
   return{patient:identity?.[1]||pm?.[1]||(safeFilePatient&&!/Onconomics/i.test(safeFilePatient)?safeFilePatient:""),dob:identity?.[2]||pm?.[2]||"",disease,stage,ctc,date:reportDate(all),vial,specimen,summaryPage:Number(summaryRecord?.page)||1,sourceSummaryPage:Number(summaryRecord?.sourcePageNumber??summaryRecord?.page)||1}
 }
 ...
 function splitReportedList(value){
   const raw=String(value||"").replace(/\s+/g," ").trim(),out=[];let buf="",depth=0;
   for(const ch of raw){
     if(ch==="(")depth++;
   ...
   return out.map(v=>v.replace(/^[\s:.-]+|[\s:.;]+$/g,"").trim()).filter(Boolean)
 }
 function explicitSummaryList(source,label,nextLabels){
   // label/nextLabels are regex fragments controlled by this parser. Do not
   // escape their \\s tokens, otherwise the boundary becomes literal text and
   // swallows the remainder of REPORT SUMMARY.
   const next=(nextLabels||[]).join("|");
   const boundary=next?`(?=\\n\\s*(?:${next})\\s*:|\\n\\s*(?:${next})\\b|$)`:"(?=\\n|$)";
   const re=new RegExp(`${label}\\s*:\\s*([\\s\\S]*?)${boundary}`,"i"),m=String(source||"").match(re);
   if(!m)return[];
   return splitReportedList(m[1].replace(/\n+/g," "))
 }
 function summaryData(text){
   ...
   // v10.319: Report Summary is source truth. Read the printed Over-expression
   // and Down-regulation lists verbatim before using any legacy catalogue inference.
   const explicitOver=explicitSummaryList(source,"Over\\s+expression",["Down\\s+regulation","NATURAL\\s+SUBSTANCES\\s+SENSITIVITY","Conclusion\\s+Results"]);
   const explicitDown=explicitSummaryList(source,"Down\\s+regulation",["NATURAL\\s+SUBSTANCES\\s+SENSITIVITY","Conclusion\\s+Results"]);
   ...
   const over=explicitOver.length?explicitOver:findNames(expressionNames),down=explicitDown.length?explicitDown:findNames(downNames);
   ...
   const classIExact=classI.map(name=>exactSourceNaturalLabel(name,naturalBlock));
   const classIIExact=classII.map(name=>exactSourceNaturalLabel(name,naturalBlock));
   const classIIIExact=classIII.map(name=>exactSourceNaturalLabel(name,naturalBlock));
   return{
    ...
    naturalClassI:classI.join(", "),naturalClassII:classII.join(", "),naturalClassIII:classIII.join(", "),naturalClassIExact:classIExact.join(", "),naturalClassIIExact:classIIExact.join(", "),naturalClassIIIExact:classIIIExact.join(", "),
 ...
 function mergePrintedLabelGuard(target,template,visual,pageText){
   // v10.319: a printed percentage label is more faithful than bar geometry
   // when it is clearly tied to the same x-slot. Geometry remains the fallback
   // when OCR damages or misses the label.
   const resistance=/Resistance\s+Factors/i.test(String(pageText||""));
   for(const [name,label] of Object.entries(template||{})){
     const lv=Number(label?.value),geometry=visual?.[name],gv=Number(geometry?.value),confidence=Number(label?.confidence||0);
     if(!Number.isFinite(lv)||lv<-100||lv>100)continue;
     const printed=/%/.test(String(label?.sourceLine||""));
     const close=Number.isFinite(gv)&&Math.abs(lv-gv)<=3;
     const strong=confidence>=80;
     if(!(printed&&(close||resistance)||resistance&&strong))continue;
     mergeValues(target,{[name]:{...label,confidence:Math.max(98,confidence),evidenceType:"LTX_V10_319_PRINTED_LABEL_GUARD",sourceLine:`v10.319 printed-label guard: ${label.sourceLine||`${name} ${lv}%`}`,candidates:[...(label?.candidates||[label]),...(geometry?.candidates||[geometry].filter(Boolean))]}},{preferSource:true})
   }
 }
 ...
 function canonicalGeneTableValues(pageObjects){
   ...
   for(const page of pageObjects){
     ...
     for(const row of rows){
       ...
       const fidelityFunction=sourceFidelityFunctionLabel(matchedName,functionCell);
       out[matchedName]={
         value,page:page.page,confidence:99,
         sourceLine:`Canonical gene table: ${nameCell} | ${fidelityFunction||functionCell} | ${riskCell} | ${relatedCell} | ${value}% | ${rowOutcomeCell||outcomeCell}`,
         evidenceType:"LTX_V15_2_CANONICAL_GENE_TABLE",
         reportedName:nameCell,
         sourceDisplayName:sourceFidelityGeneLabel(matchedName,nameCell),
         sourceRawText:textOf(row.items),
         function:fidelityFunction||functionCell,
         clinicalRisk:riskCell,
         related:relatedCell,
         outcome:rowOutcomeCell||outcomeCell,
         mainTopic:geneMainTopicForPage(page.text,matchedName),
         subGroup:fidelityFunction||functionCell||"",
         tableRowY:row.y,
         candidates:[]
       }
     }
 ...
 function reconcileStoredResults(rows,pages){
   ...
   const repaired=(rows||[]).map(original=>{
     ...
     const next={...row,
       name:rec.sourceDisplayName||sourceFidelityGeneLabel(matched,rec.reportedName)||matched,reported_name:rec.reportedName||row.reported_name||matched,normalized_name:matched,
 ...
 function buildRows(pages,graphic){
   ...
   const add=(name,domain,rec)=>{
    ...
    const decision=canonicalGene?{
     ...
     const displayName=domain==="GENE"?(rec?.sourceDisplayName||sourceFidelityGeneLabel(name,rec?.reportedName)||name):name;
     rows.push({
       group:domain,section:domain==="GENE"?geneSubGroup:sectionFor(name,domain),name:displayName,
 ...
 async function extractPdf(file,options={}){
   ...
   if(likely.length){
     ...
     for(let i=0;i<likely.length;i++){
       ...
       // Geometry is a robust fallback, but v10.319 gives a clearly recognised
       // printed percentage label final authority within the same chart slot.
       const visual=visualBarValues(canvas,pageText,n);
       mergeValues(graphic,visual,{preferSource:true});
       mergePrintedLabelGuard(graphic,template,visual,pageText);
 ...
 async function extractStoredPages(storedPages,options={}){
   ...
   const genes=Object.entries(canonical).map(([name,rec])=>({
     group:"GENE",section:rec.subGroup||geneSectionFor(name,rec),name:rec.sourceDisplayName||sourceFidelityGeneLabel(name,rec.reportedName)||name,
 ...
 return{
   ...
   _test:{additionalRows,visualBarValues,templateValues,mergePrintedLabelGuard,canonicalGeneTableValues,storedPageObjects,reconcileStoredResults,mergeValues,buildRows,norm,summaryData,metadata,sectionFor,naturalClass,sourceFidelityGeneLabel,sourceFidelityFunctionLabel}
 };
})();
