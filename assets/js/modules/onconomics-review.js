...
MIW.OnconomicsReview=(function(){
 ...
 function buildItems(data){
   const rows=[];
   if(data.meta.ctc){
     const sourceCtcPage=Number(data?.meta?.sourcePageMap?.[2]||data?.meta?.sourcePageMap?.["2"]||2);
     rows.push({id:MIW.Utils.uid("oncoDraft"),group:"CTC",section:"CTC count",name:"CTC count",value:Number(data.meta.ctc),unit:"cells/mL",page:2,source_page_number:sourceCtcPage,parserStatus:"VERIFIED",verificationStatus:"SOURCE_SUMMARY_VERIFIED",confidence:99,confidenceBand:"HIGH",canonicalSource:"REPORT_SUMMARY",evidenceType:"REPORT_SUMMARY_CTC",reviewRecommended:false,conflict:false,evidence:[{value:Number(data.meta.ctc),page:2,source_page_number:sourceCtcPage,confidence:99,evidenceType:"REPORT_SUMMARY_CTC",sourceLine:`CTCs COUNT: ${Number(data.meta.ctc)} cells/ml`}],evidenceCount:1,reconciliationReason:"ใช้ค่า CTC count ที่พิมพ์ใน REPORT SUMMARY ของต้นฉบับ"});
   }
