...
MIW.LabEngine=(function(){
 ...
 const TESTS=[
  ...
  ["Alpha Fetoprotein (AFP)","Tumor Marker","Tumor Markers","afp"],["PIVKA II (DCP)","Tumor Marker","HCC Biomarker Panel","pivka_ii"],["GAAD Score (AFP + PIVKA II)","Tumor Marker","HCC Biomarker Panel","gaad_score"],["Beta HCG","Tumor Marker","Tumor Markers","beta_hcg"],["CA 125 (Ovary Cancer)","Tumor Marker","Tumor Markers","ca_125"],["CA 15-3 (Breast Cancer)","Tumor Marker","Tumor Markers","ca_15_3"],["CA 19-9 (Digestive Tract)","Tumor Marker","Tumor Markers","ca_19_9"],["Carcinoembryonic Antigen (CEA)","Tumor Marker","Tumor Markers","cea"],["CRP (C-Reactive Protein High Sens.)","Immunology","Inflammation","crp_hs"],["NT-ProBNP","Immunology","Cardiac Biomarkers","nt_probnp"],
 ...
 const unitRe=/^(?:g\/(?:d?L)|mg\/(?:d?L)|[unµμ]?g\/(?:d?L|mL|L)|[pmunµμ]?mol\/L|mOsm\/L|ng\/(?:dL|mL|L)|pg\/mL|mIU\/mL|mAU\/mL|[uµμ]IU\/mL|U\/mL|U\/L|mmol\/mol|mm\/hr|fL|pg|%|L\/L|\/100\s*WBC|S\/CO|s|sec|secs|seconds?|cells?\/HPF|cells?\/(?:uL|µL)|\/mm\^?3|cells\/mm\^?3|10\^?[369](?:\/(?:L|uL|µL|mm\^?3))?|10\^?[369]\s+cells\/mm\^?3)$/i;
 ...
 function defAliases(def){
   const byCode={
     ...
     afp:["AFP","Alpha Fetoprotein","Alpha-Fetoprotein"],pivka_ii:["PIVKA II","PIVKA-II","PIVKA 2","DCP","Des-gamma-carboxy prothrombin"],gaad_score:["GAAD Score (AFP + PIVKA II)","GAAD Score","GAAD"],
 ...
 function inlinePhotoRows(page){
   ...
   const units="(?:mmol\\s*\\/\\s*[Ll]|mg\\s*\\/\\s*d[Ll]|g\\s*\\/\\s*[Ll]|g\\s*\\/\\s*d[Ll]|[uµ]?mol\\s*\\/\\s*[Ll]|m?Osm\\s*\\/\\s*[Ll]|ng\\s*\\/\\s*m[Ll]|mIU\\s*\\/\\s*m[Ll]|mAU\\s*\\/\\s*m[Ll]|U\\s*\\/\\s*[Ll]|%|fL|pg|10\\^?[369]\\s*\\/\\s*[Ll])";
 ...
 function normalizeMarkerPhotoNumber(raw){
   let value=clean(raw).replace(/\s+/g,"");
   if(/^\d{1,3}(?:,\d{3})+(?:\.\d+)?$/.test(value))value=value.replace(/,/g,"");
   else if(/^\d+,\d+$/.test(value))value=value.replace(",",".");
   return value
 }
 function gaadTumorMarkerPhotoRows(page){
   if(page.captureType!=="PHOTO")return[];
   const output=[],seen=new Set();
   const configs=[
     {code:"gaad_score",alias:/^\s*GAAD\s*Score(?:\s*\(\s*AFP\s*\+\s*PIVKA\s*(?:II|2)\s*\))?/i},
     {code:"afp",alias:/^\s*AFP\b/i},
     {code:"pivka_ii",alias:/^\s*PIVKA\s*[- ]?\s*(?:II|2)\b/i}
   ];
   const numberToken='(?:[<>]=?\\s*)?(?:\\d{1,3}(?:,\\d{3})+(?:\\.\\d+)?|\\d+(?:[.,]\\d+)?)';
   const rangeRe=/(?:[<>]=?\s*)?\d+(?:[.,]\d+)?\s*[-–]\s*\d+(?:[.,]\d+)?/g;
   const unitFind=/(?:mAU\s*\/\s*mL|ng\s*\/\s*mL)/i;
   for(const pass of photoPasses(page)){
     const lines=String(pass.text||"").split(/\n/).map(clean).filter(Boolean);
     for(const config of configs){
       const def=TESTS.find(item=>item[3]===config.code);if(!def)continue;
       for(const line of lines){
         const aliasMatch=line.match(config.alias);if(!aliasMatch)continue;
         const tail=clean(line.slice((aliasMatch.index||0)+aliasMatch[0].length));
         const valueMatch=tail.match(new RegExp(numberToken,'i'));if(!valueMatch)continue;
         const value=normalizeMarkerPhotoNumber(valueMatch[0]);
         const after=clean(tail.slice((valueMatch.index||0)+valueMatch[0].length));
         const flag=(after.match(/\b(?:HH|LL|H|L)\b/i)||[])[0]?.toUpperCase()||"";
         const unit=(after.match(unitFind)||[])[0]||"";
         const ranges=[...after.matchAll(rangeRe)].map(m=>m[0]);
         const reference=clean(ranges.at(-1)||"").replace(/,/g,".");
         const row=makeRow(def,page.pageNumber,value,unit,flag,reference,line,{allowRepair:false});
         if(!row)continue;
         const key=`${config.code}|${value}|${reference}`;if(seen.has(key))continue;seen.add(key);
         row.confidence=Math.min(reference?96:88,Number(pass.confidence||80));
         row.parse_issue=row.confidence<80||!reference;
         row.photo_ocr=true;row.ocr_pass=pass.mode;row.source_profile="GAAD_HCC_TUMOR_MARKERS";
         row.reported_name=aliasMatch[0].trim();output.push(row)
       }
     }
   }
   return output
...
// v10.304 — Global primary coherence lock.  InBody's Weight / Fat Mass / BMI / PBF
 ...
 function photoTableRows(page){
   ...
   for(const pass of photoPasses(page)){
     ...
     for(const def of TESTS){
       const gaadPanel=/GAAD\s*SCORE|PIVKA[\s-]*(?:II|2)/i.test(String(pass.text||""));
       if(gaadPanel&&["gaad_score","afp","pivka_ii"].includes(def[3]))continue;
   ...
   const reconstructed=bnhDocument
     ...
     :specializedDocument
       ...
       :page.captureType==="PHOTO"
         ?[...gaadTumorMarkerPhotoRows(page),...panelAwareRows(page),...photoTableRows(page)]
         :[];
   for(const def of TESTS){
     if(def[1]==="Urinalysis"&&!urinePage)continue;
     if(def[1]==="Clinical Microscopy"&&!stoolPage)continue;
