...
MIW.MixedClinicalPacket=(function(){
  ...
  const RULES=[
    {type:"LABORATORY",terms:[
      ...
      [/COLLECTED\s+DATE/i,22],[/REFERENCE\s+RANGE/i,18],[/\bUNITS?\b/i,8],[/LABORATORY\s+REPORT/i,28],
  ...
  function strongLabEvidence(page={}){
    ...
    if(/LABORATORY\s+REPORT|COMPLETE\s+BLOOD\s+COUNT|BLOOD\s+CHEMISTRY|HEMATOLOGY|COAGULATION/i.test(raw))return true;
    ...
    if(/TEST\s+NAME/i.test(raw)&&/RESULT/i.test(raw)&&/REFERENCE\s+RANGE/i.test(raw)&&/(?:PSA|CREATININE|PLATELET|ALBUMIN|WBC)/i.test(raw))return true;
